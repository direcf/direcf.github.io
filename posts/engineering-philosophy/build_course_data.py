"""Generate course_data.json for the Engineering Philosophy course."""
import json
from pathlib import Path

OUTPUT_DIR = Path("/Users/1113493/Desktop/Study/Engineering-Philosophy")

chapters = []

# ============================================================
# CHAPTER 1 — Software 2.0 (Karpathy, 2017)
# ============================================================
chapters.append({
    "number": 1,
    "emoji": "🧠",
    "title": "Software 2.0",
    "titleKr": "소프트웨어 2.0 — Andrej Karpathy (2017)",
    "tldr": "프로그래밍의 본질이 '명령어 작성'에서 '데이터로 함수 발견'으로 옮겨간다는 패러다임 선언. AI 시대 모든 변화의 출발점이다.",
    "topics": ["Software 1.0 vs 2.0", "데이터셋이 곧 코드", "신경망 가중치", "패러다임 전환", "AI 시대 프로그래밍"],
    "learningGoals": [
    ...
    "overview": (
        "Andrej Karpathy가 2017년 Tesla AI Director 시절 발표한 짧지만 결정적인 글이다. 핵심 명제는 단 하나 — "
        "프로그래밍이라는 행위 자체가 바뀌고 있다는 것. 우리는 더 이상 컴퓨터에게 '어떻게 해야 하는지'를 한 줄씩 적어주지 않는다. "
        "대신 우리는 데이터를 모으고, 모델 구조를 정의하고, 옵티마이저에게 '코드를 찾으라'고 시킨다.\n\n"
        "2017년 당시엔 주로 이미지 인식·음성 처리 같은 좁은 도메인 얘기였다. 하지만 2022년 ChatGPT 이후, 이 패러다임이 "
        "일반 프로그래밍 전체를 잠식하기 시작했다. 그래서 2025년에 다시 읽으면 이 글은 단순한 트렌드 분석이 아니라 예언서다. "
        "AI 에이전트 시대 개발자가 가장 먼저 흡수해야 할 마인드셋이 여기 있다."
    ),
    "sections": [
        {
            "title": "1.1 Software 1.0 vs Software 2.0",
            "content": (
                "Software 1.0은 인간이 직접 쓴 명령어다. Python, C++, Java... 컴파일러가 읽을 수 있는 모든 코드. "
                "우리가 '이 입력이 들어오면 이렇게 처리해'를 한 줄씩 적어준다. 결과는 결정론적이고, 사람이 한 줄 한 줄 검증할 수 있다.\n\n"
                "Software 2.0은 신경망의 가중치다. 우리가 직접 쓰지 않는다. 우리는 (1) 데이터셋과 (2) 모델 구조와 (3) loss function만 "
                "정의한다. 그러면 옵티마이저(SGD, Adam 등)가 가능한 가중치 공간을 탐색해서 우리가 원하는 동작을 만드는 가중치 조합을 찾아낸다. "
                "코드는 '써지는' 것이 아니라 '발견되는' 거다.\n\n"
                "Karpathy의 한 줄 요약: 1.0은 어떻게(how)를 명시한다. 2.0은 무엇(what — 데이터 = 예시)을 명시하면, 어떻게는 기계가 찾는다."
            )
        },
        {
            "title": "1.2 왜 이 변화가 일어나는가",
            "content": (
                "이유는 단순하다. 인간의 직관과 명시적 규칙으로는 너무 복잡한 함수가 세상에 너무 많기 때문이다. "
                "'이 픽셀들이 고양이인가?'를 if-else로 적을 방법이 없다. 수십 년간 컴퓨터 비전 연구자들이 시도했지만 전부 실패했다.\n\n"
                "그런데 1만 장의 고양이 사진을 보여주면 신경망은 그 함수를 학습한다. 'how' 를 인간이 짜내려는 시도를 포기하고, "
                "데이터를 통해 'what'만 보여주면 기계가 알아서 'how'를 찾는다는 사실이 핵심이다.\n\n"
                "Karpathy는 2017년 시점에 이미 음성 인식, 이미지 인식, 음성 합성, 기계 번역, 게임 플레이, 로보틱스 등 도메인이 모두 "
                "1.0에서 2.0으로 넘어갔다고 지적한다. 2025년엔 여기에 코드 생성, 글쓰기, 추론, 에이전트 작업이 추가됐다."
            )
        },
        {
            "title": "1.3 Software 2.0의 장점",
            "content": (
                "**계산 동질성(homogeneity)**: 거의 모든 신경망이 비슷한 연산(행렬곱, 활성화 함수)으로 구성된다. 그래서 GPU·TPU 같은 "
                "전용 하드웨어에 자연스럽게 최적화된다. Software 1.0은 분기·반복·메모리 접근이 천차만별이라 이런 최적화가 어렵다.\n\n"
                "**지속적 학습**: 새 데이터로 미세조정만 하면 동작이 갱신된다. 1.0 코드 리팩토링과는 비교가 안 될 만큼 가볍다. "
                "이게 LLM이 매년 더 똑똑해지는 이유다.\n\n"
                "**모듈 결합의 자유**: 2.0 모듈들을 학습 단계에서 함께 미분 가능하게 결합할 수 있다. 시각 인코더와 언어 모델을 붙여 "
                "vision-language model을 만드는 식. 1.0에서는 API 호출의 합으로만 가능하지만, 2.0에선 끝-끝 학습이 가능하다.\n\n"
        ...
        },
        {
            ...
            "content": (
                ...
                "**데이터 분포 의존성**: 학습 데이터와 다른 분포가 들어오면 무너진다. 'silent failure' — 틀린 답을 자신 있게 내는 게 가장 무섭다. "
                "LLM의 hallucination이 이 카테고리다.\n\n"
                "**디버깅 방식이 완전히 다르다**: 1.0은 로그·breakpoint·스택 추적. 2.0은 데이터셋 큐레이션, loss 그래프, 활성화 분석, "
                "임베딩 공간 시각화. 시니어가 주니어를 평가할 때 이 차이를 인지하는지가 핵심 지표다."
            )
        },
        {
            "title": "1.5 2025년에서 다시 읽기",
            "content": (
                "Karpathy가 2017년에 쓴 'Software 2.0'은 좁은 도메인 얘기였다. 그러나 LLM의 등장은 그의 예측을 일반 프로그래밍 전체로 확장시켰다.\n\n"
                "이제 '프롬프트 엔지니어링'이 부분적으로 코드 작성을 대체했고, 그 발전형인 'context engineering'(맥락을 설계하는 기술)이 "
                "주목받고 있다. 자연어가 직접 실행 가능한 명세가 되고 있다는 점에서, Software 2.0의 정신은 더 깊이 파고들고 있다.\n\n"
                "그가 2025년에 발표한 'Software 3.0' (이 코스의 마지막 장) 이 바로 이 글의 자연스러운 후속편이다. "
                "10년에 걸친 한 사람의 일관된 사고 흐름을 따라간다는 점에서, 이 코스 전체의 출발점이 되기에 적합하다."
            )
        }
    ],
    "analogy": {
        "title": "조각가 vs 정원사",
        "content": (
            "Software 1.0 = 조각가. 한 끌 한 끌 깎아 형상을 만든다. 모든 결, 모든 흠집이 조각가의 의도이고, 어떤 부분이 왜 그렇게 됐는지 설명할 수 있다. "
            "재현 가능하고 검증 가능하지만, 만들 수 있는 형태가 인간의 손과 시간에 묶여 있다.\n\n"
            "Software 2.0 = 정원사. 토양(데이터)을 고르고 씨앗(모델 구조)을 심고 햇빛과 물(컴퓨팅)을 준다. 정확히 어느 가지가 어디로 뻗을지는 "
            "정원사도 모른다. 하지만 적절한 조건을 주면 인간이 손으로는 절대 만들 수 없는 복잡한 구조가 자라난다.\n\n"
            "조각가는 자신이 만든 모든 결을 설명할 수 있다. 정원사는 못 한다. 그래서 정원사는 더 강력하면서 동시에 더 위험하다. "
            "AI 시대 개발자는 두 역할을 다 할 줄 알아야 하지만, 점점 정원사 쪽으로 무게중심이 이동하고 있다."
        )
    },
    "codeExample": {
        "language": "python",
        ...
        "walkthrough": (
            "1.0 버전에서 '코드'는 if/keywords다. 새 스팸 패턴(예: 암호화폐 사기)이 등장하면 keywords 리스트를 손으로 늘려야 한다. "
            "2.0 버전에서 '코드'는 training_data다. 같은 모델 구조 그대로 새 데이터로 재학습하면, 모델은 새 패턴을 알아서 학습한다. "
            "model 코드는 한 줄도 안 바뀐다 — 데이터셋이 진짜 코드라는 Karpathy의 명제가 이 한 줄로 보인다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "AI 시대 시니어 엔지니어는 1.0과 2.0 중 어느 쪽으로 풀어야 하는지 판단하는 능력을 본다. 모든 걸 LLM에 던지는 것도, "
            "모든 걸 직접 쓰는 것도 둘 다 미숙함의 신호다."
        ),
        ...
        "masteryVsFamiliar": (
            "Familiar 수준은 'Karpathy의 글'을 들어봤고 1.0/2.0의 개념을 외우고 있다. Mastery 수준은 자기 프로젝트에서 '여기는 2.0이 낫고 "
            "저기는 1.0이 나아요'를 근거와 함께 설명할 수 있고, 2.0 시스템의 운영·모니터링·평가 체계를 설계할 수 있다."
        )
    },
    "keyTakeaways": [
        {"title": "코드의 정의가 바뀌었다", "content": "'명령어'에서 '학습된 함수'로. 둘 다 코드다."},
        {"title": "데이터셋이 진짜 소스 코드", "content": "2.0에서 동작을 바꾸려면 데이터를 바꾼다."},
        {"title": "1.0과 2.0은 공존한다", "content": "어느 한쪽으로 몰빵하지 않는 게 시니어의 표식."},
        {"title": "디버깅 도구가 완전히 다르다", "content": "loss 그래프·임베딩 시각화·데이터 큐레이션이 새 stacktrace다."},
        {"title": "Silent failure가 가장 무섭다", "content": "틀린 답을 자신 있게 내는 게 LLM의 본질적 위험."},
        {"title": "정원사 마인드", "content": "조건을 만들고 결과를 모니터링한다. 모든 결을 통제할 수 없다."},
        {"title": "2017년의 예언", "content": "ChatGPT 이전에 이미 일반 프로그래밍의 미래를 그렸다."},
        {"title": "Software 3.0의 씨앗", "content": "이 글의 정신이 Karpathy의 2025년 '컴퓨터로서의 LLM'으로 이어진다."}
    ]
})

# ============================================================
# CHAPTER 2 — Manual Work is a Bug (Limoncelli, 2018)
# ============================================================
chapters.append({
    "number": 2,
    "emoji": "🔧",
    "title": "Manual Work is a Bug",
    "titleKr": "수작업은 버그다 — Thomas Limoncelli (2018)",
    "tldr": "모든 수작업은 흔적을 남겨야 한다. 자동화는 점진적 게이지의 바늘을 매번 오른쪽으로 미는 마인드셋이다.",
    "topics": ["Always Be Automating", "자동화 게이지", "Snippet → Script → Self-service → Autonomous", "Leftover Principle", "Stone Soup 협업"],
    "learningGoals": [
    ...
    "overview": (
        "Stack Overflow의 시스템 관리자 Thomas Limoncelli가 ACM Queue에 쓴 짧은 칼럼. 한 줄 요약은 도발적이다 — "
        "'모든 수작업은 버그다.' 진짜 뜻은 수작업 자체가 잘못이 아니라 '흔적을 남기지 않는 반복 수작업'이 버그라는 것이다.\n\n"
        "이 글은 SRE/DevOps 매니페스토로 자주 인용된다. AI 에이전트 시대엔 더더욱 중요해졌다. 왜냐하면 LLM은 '문서화되고 "
        "snippet화된' 절차를 가장 잘 수행하고, '머릿속에만 있는 절차'는 전혀 수행하지 못하기 때문이다. 자동화를 위한 자동화가 "
        "AI 에이전트의 발판이 된다."
    ),
    "sections": [
        {
            "title": "2.1 두 가지 원칙",
            "content": (
                "**원칙 ①**: 모든 수동 작업은 '작업 완료 + 시스템 개선'이라는 이중 목적을 가져야 한다. "
                "서버 재시작 티켓을 처리했다면 '재시작 완료'만 남으면 안 되고, 다음번에 더 쉬워질 무언가 — 문서 한 줄, snippet 하나, 스크립트 한 함수 — 가 같이 남아야 한다.\n\n"
                "**원칙 ②**: 산출물(artifact)을 만들거나 기존 산출물을 개선하지 않는 수동 작업은 '허용되어선 안 된다'. "
                "Limoncelli는 이 표현을 강하게 쓴다. 작업을 끝낼 때 시스템에 무언가가 '쌓이지' 않으면 그 작업은 다음에도 똑같은 비용으로 반복된다.\n\n"
                "이 두 원칙은 단순해 보이지만 실천하기 어렵다. 바쁠 때 '일단 끝내자'의 유혹이 강하기 때문이다. 시니어와 주니어를 가르는 건 "
                "이 유혹에 흔들리지 않는 습관이다."
            )
        },
        {
            "title": "2.2 아날로그 게이지 비유",
            "content": (
                "Limoncelli의 가장 빛나는 인사이트. 자동화를 '됐다/안 됐다'의 이분법이 아니라 옛날 자동차의 아날로그 게이지처럼 "
                "연속 스펙트럼으로 보자는 것. 왼쪽 끝은 완전 수동, 오른쪽 끝은 완전 자율.\n\n"
                "중요한 건 '바늘이 어디 있느냐'가 아니라 '오른쪽으로 움직이고 있느냐'다. 완벽한 자동화를 목표로 시작하면 너무 거대해서 "
                "시도조차 못 한다. 하지만 '이번 작업에서 바늘 한 칸 옮기기'가 목표면 매일 진짜로 굴러간다.\n\n"
        ...
        {
            "title": "2.4 Leftover Principle와 Stone Soup",
            "content": (
                "**Leftover Principle**: 자동화는 인간 판단이 필요한 어려운 부분이 아니라, 지루하고 시간만 잡아먹는 반복 부분부터 처리하라. "
                "사람의 판단이 필요한 결정은 마지막까지 남겨도 좋다. 80%만 자동화돼도 손익은 압도적이다.\n\n"
                "**Stone Soup**: 서양 동화에서 온 비유. 나그네가 돌멩이 수프를 끓이겠다며 솥에 물을 끓이자 마을 사람들이 호기심에 당근·감자·고기를 보탠다. "
                "결국 다 같이 먹을 진짜 수프가 완성된다. 메시지: 첫 단계 산출물(돌멩이 = 허접한 문서)을 git에 올려두면 동료들이 자발적으로 기여해서 "
                "혼자선 절대 만들지 못했을 팀 자산이 된다.\n\n"
                "'완벽해지면 공개하겠다'는 영원히 공개 안 한다는 뜻이다. 부끄러운 단계부터 공유하라."
            )
        },
        {
            "title": "2.5 AI 에이전트 시대에 더 중요해진 이유",
            "content": (
                "Limoncelli는 2018년에 인간 자동화를 얘기했지만, 이 정신은 AI 에이전트 시대에 두 배로 적용된다.\n\n"
                "LLM 에이전트(Claude Code, Cursor, Devin 등)는 '문서화되고 절차화된' 시스템에선 강력하지만, '머릿속에만 있는' 시스템에선 무력하다. "
                "팀의 모든 절차가 git에 markdown으로 있고, snippet으로 박혀 있다면 에이전트가 그걸 읽고 실행할 수 있다. 종이로만 굴러가는 절차는 "
                "에이전트에게 보이지 않는다.\n\n"
                "즉, Limoncelli의 5단계 게이지는 이제 'AI 에이전트가 활동할 수 있는 무대를 마련하는 작업'이기도 하다. "
                "수작업 → 문서화 → 스크립트화 → AI 에이전트 호출 가능. 이게 새로운 자동화의 전체 spectrum이다."
            )
        }
    ],
    "analogy": {
        ...
        "content": (
            ...
            "자동화도 마찬가지다. 완전 수동(0)에서 완전 자율(redline) 사이의 어느 지점에서 우리는 살고 있다. 중요한 건 절대 위치가 아니라 방향이다. "
            "이번 달 바늘이 어제보다 0.5 칸 오른쪽이면 그 팀은 건강하다. 6개월간 같은 위치면 toil이 누적되고 있다.\n\n"
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "1단계에선 절차가 외부 산출물(파일)이 된다. 2단계에선 그 절차가 복붙 가능해진다. 3단계에선 한 명령으로 실행 가능해진다. "
            "4단계에선 다른 사람이 호출 가능해진다. 5단계에선 사람이 호출조차 안 한다. 각 단계는 이전 단계 위에 쌓이고, 이전 단계의 '부산물'이 "
            "다음 단계의 출발점이 된다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "SRE·DevOps·플랫폼팀에선 이 글의 정신이 사실상 채용 기준이다. AI 에이전트 시대엔 일반 개발자에게도 이 기준이 적용되기 시작했다."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'toil 줄여야지'를 알고 있다. Mastery는 자기 팀의 toil 게이지가 지금 어디 있고, 이번 분기에 어느 절차의 바늘을 "
            "어느 단계까지 옮길지 구체적 계획이 있다."
        )
    },
    "keyTakeaways": [
        {"title": "수작업 자체가 죄가 아니다", "content": "흔적 안 남기는 반복 수작업이 죄."},
        {"title": "이중 목적", "content": "작업 완료 + 시스템 개선. 둘 다 못 하면 다시 하라."},
        {"title": "게이지의 방향", "content": "위치보다 방향. 매번 한 칸씩 오른쪽으로."},
        {"title": "Leftover Principle", "content": "지루한 반복부터. 판단은 사람에게 남겨도 좋다."},
        {"title": "Stone Soup", "content": "허접해도 git에 올려라. 동료가 보탠다."},
        {"title": "5단계", "content": "Document → Snippets → Script → Self-Service → Autonomous."},
        {"title": "AI 에이전트의 발판", "content": "문서화된 절차만 에이전트가 실행할 수 있다."},
        {"title": "마인드셋이 결정적", "content": "도구·언어가 아니라 '항상 바늘을 미는' 습관이 갈림길."}
    ]
})

# ============================================================
# CHAPTER 3 — A Philosophy of Software Design (Ousterhout, 2018)
# ============================================================
chapters.append({
    "number": 3,
    "emoji": "🏛️",
    "title": "A Philosophy of Software Design",
    "titleKr": "소프트웨어 설계 철학 — John Ousterhout (2018)",
    "tldr": "복잡도(complexity)는 소프트웨어의 단 하나의 진짜 적. Deep modules, strategic 프로그래밍, 그리고 인지 부하 줄이기.",
    "topics": ["Complexity", "Deep Modules", "Strategic vs Tactical", "Cognitive Load", "Information Hiding"],
    "learningGoals": [
    ...
    "overview": (
        "스탠퍼드 대학의 John Ousterhout 교수가 자신의 CS190 강의 노트를 책으로 엮은 것. Tcl과 Raft 합의 알고리즘으로 유명한 그가 "
        "수십 년의 시스템 설계 경험을 응축한 결과물이다. 한 줄 요약: 소프트웨어의 단 하나의 진짜 적은 '복잡도(complexity)'다.\n\n"
        "이 책의 가장 강력한 개념은 'deep module' — 인터페이스는 단순하지만 내부 기능은 강력한 모듈. AI 시대엔 더더욱 중요하다. "
        "왜냐하면 LLM이 우리 코드를 읽고 수정하는 비중이 커질수록, 인터페이스가 단순하고 책임이 분명한 모듈이 에이전트의 실수를 줄이기 때문이다. "
        "Shallow module은 LLM에게도 인간에게도 적이다."
    ),
    "sections": [
        {
            "title": "3.1 복잡도란 무엇인가",
        ...
        {
            "title": "3.2 Deep Module — 이 책의 핵심 개념",
            "content": (
        ...
        {
            "title": "3.3 Strategic vs Tactical 프로그래밍",
            "content": (
                "**Tactical 프로그래밍**: 당장 작동하는 것이 목표. '일단 돌아가게 만들자.' 복잡도가 누적되는 줄도 모르고 누적된다.\n\n"
                "**Strategic 프로그래밍**: 좋은 설계가 목표. 같은 기능을 만들더라도 '미래의 자기 자신과 동료를 위해 잘 만들자.' Ousterhout는 "
                "이를 위해 매 PR마다 10-20%의 추가 시간을 투자하라고 권한다.\n\n"
                "결정적 통찰: Tactical은 단기 속도는 빠르지만 6개월 뒤엔 Strategic이 압승한다. 누적된 복잡도가 모든 변경의 비용을 끌어올리기 때문이다. "
                "수학적으로 복리(compound interest)와 같은 모양으로 손해가 누적된다.\n\n"
                "그는 실리콘밸리에서 직접 본 두 가지 패턴을 비교한다 — Tactical tornadoes(빠르게 많이 만드는 사람들)는 단기엔 영웅이지만 그들이 만든 코드가 회사의 발목을 잡고, "
                "Strategic 엔지니어들은 묵묵하게 좋은 코드를 쌓아 결국 회사의 진짜 자산이 된다."
            )
        },
        {
            "title": "3.4 Information Hiding과 그 적",
            "content": (
                "**Information hiding**: 모듈은 자신의 구현 세부사항을 숨겨야 한다. 클래식한 원칙이지만 Ousterhout는 한 단계 더 들어간다.\n\n"
                "그의 강력한 주장: 'Pass-through 메서드'는 information hiding의 적이다. 클래스 A의 메서드가 단순히 클래스 B의 메서드를 호출하기만 한다면, "
                "A가 B의 인터페이스를 그대로 노출하고 있는 셈이다. 추상화 비용은 들지만 추상화 이득은 없다. 이게 shallow module의 전형이다.\n\n"
                "또 다른 적: '시간적 분해(temporal decomposition)'. 시간 순서대로 모듈을 나누는 것. 예: parseInput → process → writeOutput. "
                "각 모듈이 같은 데이터 구조를 알아야 한다면 결합도가 강한 것이다. 모듈을 나누되 데이터 구조 지식이 한 군데에 모이도록 설계해야 한다."
            )
        },
        {
            "title": "3.5 AI 시대에 왜 더 중요한가",
            "content": (
                "AI 에이전트가 코드를 읽고 수정하는 비중이 커질수록, 모듈 설계의 품질이 에이전트의 성능을 직접 결정한다.\n\n"
                "Shallow module이 가득한 코드베이스에선 에이전트가 변경 한 줄을 위해 수십 개 파일을 읽어야 한다. context window를 다 써버린다. "
                "Deep module이 잘 분리된 코드베이스에선 에이전트가 작은 영역만 읽고 정확한 수정을 한다.\n\n"
                "즉, Ousterhout의 원칙은 AI 시대에 'LLM-친화적 설계'와 사실상 같은 말이 된다. 인간을 위해 잘 설계된 코드는 LLM에게도 잘 설계된 코드다. "
                "이게 우연이 아니다 — 두 독자 모두 '인지 부하'에 약하기 때문이다.\n\n"
                "Anthropic의 Claude Code 팀이 'good engineering = good for AI agents'라고 자주 말하는 근거가 여기 있다."
            )
        }
    ],
    "analogy": {
        "title": "지하철 노선도 vs 회로도",
        "content": (
            "도쿄 지하철 노선도는 deep abstraction의 정수다. 인터페이스(역 이름 + 노선 색)는 극도로 단순하지만, 그 뒤에 숨은 시스템 — "
            "전력·신호·차량 정비·인력 — 은 거대하다. 사용자는 그 거대함을 알 필요가 없다.\n\n"
            "반대로 회로도는 모든 와이어가 노출돼 있다. 회로도를 노선도로 들고 다니라고 하면 누구도 못 쓴다. 추상화의 가치가 0이다.\n\n"
            "코드 모듈도 같다. 사용자가 알아야 하는 인터페이스의 양(가로)을 최소화하면서, 내부에서 해주는 일(세로)을 최대화하는 게 deep module이다. "
            "AI 에이전트도 결국 사용자다. 회로도를 보여주면 길을 잃는다."
        )
    },
    "codeExample": {
        "language": "python",
        ...
        "walkthrough": (
            "Shallow 버전은 '추상화'를 흉내냈지만 사용자는 결국 파일 I/O의 내부를 다 알아야 한다. 추상화의 비용만 있고 이득은 없다. "
            "Deep 버전은 인터페이스 한 줄로 사용자에게 '나는 파일을 텍스트로 읽어준다' 외에는 알 필요 없다고 말한다. "
            "내부 복잡도(인코딩·버퍼·자원 해제)는 모두 모듈이 흡수한다. 이게 deep module의 본질이다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "이 책은 시니어 코드 리뷰의 무기다. PR 리뷰에서 'shallow module인데?' 한 마디로 잘못된 추상화를 잡아내는 데 쓰인다."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'deep module이 좋다'를 외운다. Mastery는 자기 코드베이스의 shallow module 후보를 가리킬 수 있고, "
            "왜 그것이 shallow한지, 어떻게 deep으로 바꿀지 구체적으로 설명할 수 있다."
        )
    },
    "keyTakeaways": [
        {"title": "복잡도가 유일한 적", "content": "버그·성능·보안 모두 복잡도의 자식들이다."},
        {"title": "복잡도의 세 증상", "content": "변경 증폭, 인지 부하, 미지의 미지."},
        {"title": "Deep module", "content": "좁은 인터페이스, 깊은 기능. 추상화의 진짜 모양."},
        {"title": "Shallow module은 추상화 사칭", "content": "비용만 있고 이득은 없다."},
        {"title": "Strategic > Tactical", "content": "10-20% 더 투자해서 6개월 뒤 압승."},
        {"title": "Pass-through 함수 경계", "content": "단순 위임 메서드는 information hiding의 적."},
        {"title": "시간적 분해 회피", "content": "데이터 구조 지식이 한 군데에 모이게 분리."},
        {"title": "LLM 친화적 설계", "content": "인간에게 좋은 설계는 AI 에이전트에게도 좋다."}
    ]
})

# ============================================================
# CHAPTER 4 — The Bitter Lesson (Rich Sutton, 2019)
# ============================================================
chapters.append({
    "number": 4,
    "emoji": "🪨",
    "title": "The Bitter Lesson",
    "titleKr": "쓰라린 교훈 — Rich Sutton (2019)",
    "tldr": "70년 AI 역사의 가장 큰 교훈 — 인간이 짜낸 지식보다 '연산을 활용하는 일반적 방법'이 항상 결국 이긴다.",
    "topics": ["Scaling", "Search & Learning", "Human knowledge vs Computation", "Moore's Law", "왜 LLM이 통하는가"],
    "learningGoals": [
    ...
    "overview": (
        "Rich Sutton — 강화학습의 아버지, 2024년 튜링상 수상자 — 가 2019년에 쓴 단 2페이지 짜리 짧은 글. 그러나 영향력으로는 21세기 AI "
        "에세이 중 가장 거대하다. 한 줄 요약: '인간이 자기 지식을 시스템에 주입하려 한 모든 시도는, 연산을 활용하는 일반적 방법(검색·학습)에 결국 압도당했다.'\n\n"
        "Sutton은 이것이 '쓰라리다(bitter)'고 말한다. 왜냐하면 우리가 사랑한 — 사람의 통찰을 코드로 옮기는 — 그 우아한 접근들이 매번 패배했기 때문이다. "
        "체스, 바둑, 음성 인식, 컴퓨터 비전, 그리고 지금의 LLM까지. ChatGPT와 Claude의 작동 원리를 이해하려면 이 글에서 시작해야 한다."
    ),
    "sections": [
        {
            "title": "4.1 핵심 주장",
            "content": (
                ...
                "70년 AI 연구를 돌아보면 한 패턴이 끊임없이 반복된다. (1) 연구자들이 자기 도메인 지식을 시스템에 주입한다. (2) 단기적으론 잘 된다. "
                "(3) 컴퓨팅이 더 싸지고 더 많아진다. (4) 단순한 일반적 방법(검색·학습)이 더 많은 컴퓨팅으로 무장하고 등장한다. (5) 인간 지식 기반 시스템을 압도한다. "
                "(6) 연구자들은 절망한다.\n\n"
        ...
        },
        {
            ...
            "content": (
                ...
                "**바둑 (2016, AlphaGo)**: 바둑은 검색 공간이 너무 커서 무차별이 안 통한다고 했다. 하지만 AlphaGo는 self-play 강화학습 + Monte Carlo Tree Search라는 "
                "'일반적 방법'으로 인간 지식 기반 봇들을 압도했다.\n\n"
                ...
                "**자연어 처리 (2017-2022)**: 구문 분석·시맨틱 분석을 손으로 설계하던 모든 파이프라인이 Transformer + 거대 코퍼스에 무너졌다. "
                "ChatGPT는 이 패턴의 가장 화려한 사례다."
            )
        },
        {
            "title": "4.3 무엇이 '일반적 방법'인가",
            "content": (
                "Sutton은 두 가지를 꼽는다 — 검색(Search)과 학습(Learning).\n\n"
                "검색: 가능한 공간을 알고리즘으로 탐색. 체스의 alpha-beta, 바둑의 MCTS, LLM 추론의 chain-of-thought·tree-of-thought·MCTS-decoding. "
                "검색은 '한 번에 답을 내지 말고 여러 가능성을 펼쳐서 평가하라'는 일반적 방법.\n\n"
        ...
        },
        {
            ...
            "content": (
                ...
                "단기적으론 효과 있다. 도메인 전문가의 지식을 코드화하면 빠르게 baseline을 이긴다. 논문이 나오고 학회 발표가 된다. 하지만 그 지식은 "
                "시스템의 확장을 막는다. 컴퓨팅이 10배 늘어났을 때, 일반적 방법은 자동으로 10배 강해지지만 인간 지식 기반 시스템은 '내 지식의 한계'에 갇힌다.\n\n"
                "더 심각한 건 인간 지식이 일반적 방법의 학습을 방해한다는 점이다. 사전 정의된 feature, 사전 정의된 규칙, 사전 정의된 보상은 모두 모델이 더 나은 표현을 "
                "스스로 찾는 걸 막는다. '내가 가르친 것보다 더 나은 답은 보일 수 없다.'\n\n"
                "Sutton의 결론: '컴퓨팅을 활용할 수 있는 단순하고 일반적인 방법을 만들고, 도메인 지식 주입의 유혹을 거부하라.'"
            )
        },
        {
            "title": "4.5 AI 에이전트 시대에 어떻게 적용되나",
            "content": (
                ...
                "주의: Sutton의 교훈은 '인간 지식이 항상 나쁘다'가 아니다. '컴퓨팅이 확장될 때, 인간 지식 의존도는 결국 발목이 된다'는 것. "
                "지금 당장의 작은 시스템에서 도메인 지식이 도움 되는 건 사실이지만, 그게 'AGI까지 가는 길'은 아니다."
            )
        }
    ],
    "analogy": {
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "두 함수의 '확장 가능성'을 비교하라. 규칙 기반은 컴퓨팅/데이터가 늘어도 자동으로 강해지지 않는다. 인간이 규칙을 더 써야 한다. "
            "LLM 기반은 데이터를 더 주거나, 더 강한 모델로 갈아끼우거나, 더 많은 thinking 시간을 주는 것만으로 자동으로 강해진다. "
            "Bitter Lesson은 이 비대칭이 시간이 흐르면 결정적 차이가 된다고 말한다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "AI 시스템 설계 결정마다 이 교훈을 의식하는지가 시니어와 주니어를 가른다. '나는 도메인을 안다'는 자신감이 함정이 되기 쉽다."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'Sutton의 Bitter Lesson 들어봤다'고 한다. Mastery는 자기 코드베이스의 결정 하나하나가 Bitter Lesson 쪽으로 가는지, "
            "반대로 가는지 식별할 수 있고, 결정의 ROI를 5년 시야에서 평가한다."
        )
    },
    "keyTakeaways": [
        {"title": "쓰라린 한 줄", "content": "컴퓨팅 + 일반적 방법이 매번 인간 지식을 압도했다."},
        {"title": "검색과 학습", "content": "두 가지 일반적 방법이 70년간 모든 승리를 만들었다."},
        {"title": "단기 처방, 장기 독", "content": "도메인 지식 주입은 빠른 baseline을 주지만 확장의 발목."},
        {"title": "쓰라림의 출처", "content": "연구자가 사랑한 우아한 통찰이 매번 무차별에 졌다."},
        {"title": "Moore's Law의 동맹", "content": "컴퓨팅이 싸지면 일반적 방법이 자동으로 강해진다."},
        {"title": "LLM의 이론적 기반", "content": "이 글이 ChatGPT·Claude의 작동 원리를 설명한다."},
        {"title": "엔지니어링 나침반", "content": "결정 앞에서 묻기 — '컴퓨팅이 10배면 어느 쪽이 자동으로 강해지나?'"},
        {"title": "전부냐 아니냐는 아니다", "content": "지금 작동하려면 도메인 지식이 필요하다. 하지만 그게 미래는 아니다."}
    ]
})

# ============================================================
# CHAPTER 5 — Choose Boring Technology (Dan McKinley, 2015)
# ============================================================
chapters.append({
    "number": 5,
    "emoji": "🥱",
    "title": "Choose Boring Technology",
    "titleKr": "지루한 기술을 선택하라 — Dan McKinley (2015)",
    "tldr": "당신에겐 ~3개의 'innovation token'만 있다. 어디에 쓸지 신중하라. 새로운 기술은 흥미롭지만, 지루한 기술이 당신을 지킨다.",
    "topics": ["Innovation Tokens", "Tech Selection", "Operational Cost", "Complexity Budget", "AI 시대 기술 선택"],
    "learningGoals": [
    ...
    "overview": (
        ...
        "이 글의 가치는 AI 시대에 오히려 폭증했다. 매주 새로운 LLM, 매월 새로운 에이전트 프레임워크, 매분기 새로운 RAG 라이브러리가 나오는 지금, "
        "'무엇을 도입할 것인가'보다 '무엇을 도입하지 않을 것인가'가 더 중요한 결정이 됐다. 이 글은 그 결정의 나침반이다."
    ),
    "sections": [
        {
            "title": "5.1 Innovation Token의 정의",
            "content": (
                ...
                "이 토큰은 무엇을 위해 쓰는가? 진짜로 차별화를 만드는 핵심 영역. '이걸 새 기술로 안 하면 우리 비즈니스가 안 된다' 수준의 영역. "
                "그 외 모든 곳에선 지루한 기술을 써야 한다."
            )
        },
        {
            "title": "5.2 왜 새로운 기술이 비싼가",
            "content": (
        ...
        },
        {
            ...
            "content": (
                ...
                "Boring AI 선택의 예: (a) 모델은 안정된 메이저 벤더 API (Anthropic, OpenAI). (b) 인터페이스는 단순한 messages API. "
                "(c) 도구는 잘 검증된 OS·DB·로깅. (d) 자체 에이전트 루프는 단순한 while + 함수 호출.\n\n"
                "Exciting AI를 토큰으로 살 가치가 있는 경우: 자기 비즈니스의 핵심 차별화 영역이고, 다른 방법으론 안 되며, 실패해도 되돌릴 수 있을 때. "
                "그 외 모든 곳에서는 가장 지루한 선택을 하라."
            )
        }
    ],
    "analogy": {
        "title": "포커 칩과 도박장",
        "content": (
            "당신은 칩 3개를 들고 도박장에 들어간다. 모든 테이블이 '새 기술'을 걸 수 있다. 한 테이블은 '새 데이터베이스'를 걸고 대박/쪽박을 가린다. "
            "다른 테이블은 '새 ML 프레임워크', 또 다른 테이블은 '새 에이전트 라이브러리'.\n\n"
            "칩은 3개뿐이다. 어느 테이블에 걸 것인가? 답은 '내가 진짜 알고 싶고, 잘하면 게임을 바꿀 수 있는 한두 테이블'이다. "
            "나머지 테이블에선 그냥 룰렛(boring) 빨강에 1배수 베팅을 한다. 안전하고 예측 가능하다.\n\n"
            "초보 도박꾼은 모든 테이블에 한 칩씩 다 건다. 흥미진진하지만 칩이 빨리 사라진다. 시니어는 한두 테이블에 집중한다. "
            "엔지니어링도 같다. 흥미로움이 곳곳에서 유혹할 때, 'innovation token이 3개뿐이다'를 기억하라."
        )
    },
    "codeExample": {
        "language": "python",
        ...
        "walkthrough": (
            "exciting 버전은 디버깅·모니터링·롤백이 다 위험하다. 한 부분이 망가지면 전체 파이프라인이 잘 안 보인다. "
            "boring 버전은 PostgreSQL 로그, pg_stat, Python logging 같은 잘 알려진 도구로 디버깅 가능하다. innovation token은 '진짜로 차별화를 만드는' 한 곳 — "
            "LLM 모델 자체 — 에만 썼다. 같은 결과를 내면서 운영 비용은 압도적으로 낮다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "시니어 엔지니어가 '도입하지 않는' 결정을 자랑스럽게 설명할 수 있는지가 핵심 지표. 모든 흥미로운 도구를 채택하는 사람은 위험 신호."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'boring tech가 좋다'를 모토처럼 외운다. Mastery는 자기 회사가 어느 영역에서 토큰을 써야 차별화가 되는지 명확히 알고, "
            "그 외 모든 영역에서 의도적으로 지루함을 유지한다."
        )
    },
    "keyTakeaways": [
        {"title": "토큰은 ~3개", "content": "더 많이 쓰면 운영 비용이 폭발한다."},
        {"title": "Boring ≠ Bad", "content": "Boring = Predictable. 알려진 위험을 운영한다."},
        {"title": "새 기술의 진짜 비용", "content": "운영·디버깅·고용·롤백. 학습 시간은 빙산의 일각."},
        {"title": "핵심에만 토큰", "content": "차별화 영역에만 새 기술. 그 외엔 가장 지루한 선택."},
        {"title": "Unknown failure modes", "content": "새 기술은 어디서 어떻게 망가지는지 모른다."},
        {"title": "AI 시대 더 중요", "content": "프레임워크가 매주 등장·소멸하는 시대일수록 자제가 자산."},
        {"title": "거절도 결정", "content": "도입하지 않은 기술이 시스템을 살린다."},
        {"title": "시간이 지루함을 만든다", "content": "오늘의 안정된 기술도 한때 흥미로웠다. 시간이 무료 가치를 더한다."}
    ]
})

# ============================================================
# CHAPTER 6 — Tidy First? (Kent Beck, 2024)
# ============================================================
chapters.append({
    "number": 6,
    "emoji": "🧹",
    "title": "Tidy First?",
    "titleKr": "정리부터? — Kent Beck (2024)",
    "tldr": "큰 리팩토링은 거의 항상 실패한다. 작은 'tidying'을 변경 전·중·후에 분리해서 적용하라. 이게 진짜 점진적 설계다.",
    "topics": ["Tidying vs Refactoring", "구조 변경과 동작 변경의 분리", "Small Steps", "Empirical Design", "AI 페어 프로그래밍"],
    "learningGoals": [
    ...
    "overview": (
        ...
        "이 책의 정수: 큰 리팩토링은 거의 항상 실패한다. 너무 큰 변경은 리뷰가 어렵고, 머지가 어렵고, 회귀가 일어나기 쉽고, 무엇보다 중간에 멈추기 어렵다. "
        "대신 'tidying' — 5분 안에 끝나는 작은 구조 정리 — 를 변경의 전·중·후에 자유롭게 끼워 넣으라는 것. AI 시대엔 이 패턴이 페어 프로그래밍 워크플로의 기본 단위가 됐다."
    ),
    "sections": [
        {
            "title": "6.1 Tidying이란 무엇인가",
            "content": (
                "Tidying = 동작은 그대로 두고 구조만 살짝 개선하는 작은 변경. 함수 이름 바꾸기, 변수 추출, 중복 한 줄 제거, 들여쓰기 맞추기, 매직 넘버를 상수로.\n\n"
                "Refactoring과 어떻게 다른가? Beck은 둘 다 '동작 보존 구조 변경'이지만, Tidying은 '5분 안에 끝나는 작은 단위'를 가리킨다. "
                "Refactoring은 며칠 걸리는 큰 단위. 이 분리가 핵심이다.\n\n"
                "Tidying의 본질: 작아서 안전하다. 작아서 리뷰가 쉽다. 작아서 머지 충돌이 적다. 작아서 롤백이 쉽다."
            )
        },
        {
            "title": "6.2 구조 변경과 동작 변경의 분리",
            "content": (
                ...
                "왜 분리하나? (1) 리뷰어가 정신적 작업을 절반으로 줄일 수 있다. (2) 회귀 디버깅이 압도적으로 쉬워진다 — '이 버그가 구조 변경에서 왔나? 동작 변경에서 왔나?' "
                "(3) 롤백 시 '동작은 살리고 구조만 되돌리기'가 가능하다.\n\n"
                "Beck의 한 마디: '같은 PR에 둘이 섞이면 둘 다 위험해진다.'"
            )
        },
        {
            "title": "6.3 Tidy First / After / Never",
            "content": (
        ...
        },
        {
            ...
            "content": (
                ...
                "하지만 'tidy first' 접근으로 분리하면: (1) 인간이 변경 의도를 정한다. (2) 에이전트에게 '이 기능 변경을 위해 먼저 이 함수를 추출해 줘' 같은 작은 tidying을 시킨다. "
                "(3) 그 tidying을 커밋한다. (4) 그 위에서 실제 동작 변경을 진행한다.\n\n"
                "이 워크플로의 미덕: 매 단계가 작아서 에이전트가 정확히 처리한다. 회귀 시 어느 단계에서 깨졌는지 추적이 쉽다. "
                "Beck의 50년 통찰이 AI 시대 에이전트 협업 패턴이 됐다는 게 흥미로운 우연이다 — 우연이 아니라, 좋은 엔지니어링은 인간과 AI 모두에게 좋기 때문이다."
            )
        }
    ],
    "analogy": {
        "title": "주방 셰프의 mise en place",
        "content": (
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "한 PR로 합쳤을 땐 리뷰어가 '구조와 동작'을 동시에 머릿속에 들고 검토해야 한다. 분리하면 각 PR이 한 가지만 검증하면 된다. "
            "특히 6개월 뒤 회귀 디버깅 시, git bisect가 정확히 'PR 2가 깨뜨렸다'고 가리킨다 — PR 1은 동작이 같다고 보장된 PR이기 때문이다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "PR 흐름과 커밋 위생을 보면 그 개발자가 Beck의 원칙을 이해하는지 즉시 보인다. 시니어 코드 리뷰의 1순위 검토 포인트."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'small commits 좋다'를 안다. Mastery는 자기 PR이 매번 'tidy first / behavior change / tidy after' 중 어떤 것인지 명확히 말할 수 있고, "
            "팀의 리뷰 흐름이 이 분리를 강제하도록 설계한다."
        )
    },
    "keyTakeaways": [
        {"title": "Tidying ≠ Refactoring", "content": "5분 안에 끝나는 작은 단위 vs 며칠 걸리는 큰 단위."},
        {"title": "구조와 동작 분리", "content": "한 PR엔 하나만. 두 개 섞이면 둘 다 위험."},
        {"title": "Make the change easy", "content": "정리가 변경을 쉽게 만든다. 정리 → 변경 순서."},
        {"title": "큰 리팩토링은 실패한다", "content": "Beck의 50년 관찰. 거의 모든 사례에서."},
        {"title": "Tidy First/After/Never", "content": "매번 의식적 결정. 디폴트 없음."},
        {"title": "Small steps", "content": "각 단계가 안전하고 되돌릴 수 있게."},
        {"title": "AI 에이전트의 단위", "content": "작은 tidy = 에이전트가 정확히 처리하는 작업 크기."},
        {"title": "git history가 자산", "content": "분리된 커밋이 6개월 뒤 회귀 디버깅을 살린다."}
    ]
})

# ============================================================
# CHAPTER 7 — What We Learned from a Year of Building with LLMs (Yan, Bischof, Sharma, Howard et al., 2024)
# ============================================================
chapters.append({
    "number": 7,
    "emoji": "🛠️",
    "title": "What We Learned from a Year of Building with LLMs",
    "titleKr": "LLM으로 1년간 빌드하며 배운 것 — Eugene Yan et al. (2024)",
    "tldr": "여섯 명의 LLM 실무자가 1년간 현장에서 배운 전술·운영·전략. 데모는 쉽고 프로덕션은 어렵다.",
    "topics": ["Prompt Engineering", "Evals", "RAG", "Guardrails", "Operational Reality", "Demo vs Production"],
    "learningGoals": [
    ...
    "overview": (
        "Eugene Yan, Bryan Bischof, Charles Frye, Hamel Husain, Jason Liu, Shreya Shankar 여섯 명이 2024년 6월에 O'Reilly에 공동 발표한 시리즈. "
        "각자 1년간 LLM으로 프로덕트를 만들고 운영하며 배운 교훈을 전술·운영·전략 세 부분으로 나눠 정리했다.\n\n"
        "이 글이 특별한 이유: 학계가 아니라 현장이다. 데모 영상은 쉽다. 평일 새벽 3시에 모델이 이상한 답을 내고 SLA가 깨졌을 때 무엇을 하는가가 진짜 문제다. "
        "이 글은 그 진짜 문제들에 대한 매뉴얼에 가깝다. AI 시대 엔지니어가 가장 빨리 읽어야 할 단일 문서다."
    ),
    "sections": [
        {
            "title": "7.1 전술 — 프롬프트 엔지니어링의 실전",
            "content": (
        ...
        {
            "title": "7.3 RAG의 함정과 실전",
            "content": (
                "**Chunking이 가장 어렵다**: 문서를 어떻게 자르느냐가 RAG 성능의 60%를 결정한다. 너무 짧으면 맥락 손실, 너무 길면 관련 없는 정보 섞임. "
                "도메인별로 다른 chunking 전략이 필요하다. 매뉴얼은 섹션 단위, 코드는 함수 단위, 채팅 로그는 turn 단위.\n\n"
        ...
        {
            "title": "7.4 운영 — Hallucination·Guardrails·관측",
            "content": (
    ...
    "analogy": {
        "title": "모터쇼와 24시간 르망 레이스",
        "content": (
            "모터쇼는 화려하다. 컨셉카가 빛나고, 미래가 다가오는 것 같다. 0-100km 가속, 최고 속도, 스타일. 디테일은 가렸다.\n\n"
            "르망 24시간 레이스는 다르다. 같은 차로 24시간을 달려야 한다. 연비, 신뢰성, 정비 시간, 타이어 마모, 비 오는 트랙에서의 그립, 운전자 피로. "
            "모터쇼의 우승작이 르망에서 1바퀴 만에 퍼져버린다. 르망의 우승작은 모터쇼에선 평범해 보인다.\n\n"
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "구조화된 출력(Pydantic 스키마) + few-shot 예시(평가셋과 공유) + 실패 로깅(평가셋 환류). 이 세 요소가 LLM 프로덕트의 '심장 박동'이다. "
            "이 패턴 없이 free-form 텍스트로 받으면 파싱 오류·정확도 저하·디버그 불가 삼중고에 빠진다. 글의 핵심 권고가 이 한 함수에 응축돼 있다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "AI 엔지니어 면접에서 '평가 셋과 운영 모니터링을 어떻게 설계하시나요'가 모델 선택보다 비중이 커지고 있다."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'RAG·평가 안다'고 한다. Mastery는 자기 시스템의 evaluation 셋 크기·구성·갱신 주기를 숫자로 댈 수 있고, "
            "지난 한 달간 평가셋에서 발견한 회귀와 그 대응을 설명할 수 있다."
        )
    },
    "keyTakeaways": [
        {"title": "Eval이 심장", "content": "평가 없으면 개선 없다. 모델 선택보다 먼저 평가셋."},
        {"title": "Few-shot + 구조화 출력", "content": "정확도·파싱·디버그 모두 개선."},
        {"title": "RAG > Fine-tuning, 거의 항상", "content": "지식 주입은 RAG, 양식 변경은 fine-tuning."},
        {"title": "Hybrid retrieval + reranker", "content": "벡터 단독은 약하다. BM25 + 벡터 + reranker."},
        {"title": "Guardrails 계층화", "content": "입력·출력·의미 세 단. 한 단으로는 부족."},
        {"title": "Silent failure 인지", "content": "틀린 답을 자신 있게 — 가장 무서운 실패 양식."},
        {"title": "Trace 우선 관측", "content": "디버깅의 99%가 trace에서 끝난다."},
        {"title": "Demo와 프로덕션은 다른 종목", "content": "80% vs 99.9%. 같은 시스템 아님."}
    ]
})

# ============================================================
# CHAPTER 8 — Building Effective Agents (Anthropic, 2024)
# ============================================================
chapters.append({
    "number": 8,
    "emoji": "🤖",
    "title": "Building Effective Agents",
    "titleKr": "효과적인 에이전트 만들기 — Anthropic (2024)",
    "tldr": "워크플로(workflow)와 에이전트(agent)는 다르다. 대부분의 문제는 단순한 워크플로로 풀린다. 진짜 에이전트는 마지막에 신중하게.",
    "topics": ["Augmented LLM", "Workflow vs Agent", "Tool Use", "Prompt Chaining", "Routing", "Parallelization", "Orchestrator", "Evaluator"],
    "learningGoals": [
    ...
    "overview": (
        "Anthropic이 2024년 12월에 발표한 가이드. AI 에이전트 광풍 속에서 'agent'라는 용어가 너무 마케팅화돼 의미가 흐려진 시점에, "
        "Anthropic은 정의를 다잡고 실용적 패턴을 정리했다. 단순함을 강조한다 — Sutton의 Bitter Lesson과 McKinley의 Boring Tech 정신이 모두 녹아 있다.\n\n"
        "이 글의 핵심 메시지: 에이전트를 만들기 전에, 그게 정말 에이전트여야 하는지 묻자. 대부분의 문제는 '워크플로' — 미리 정의된 단계 — 로 충분하다. "
        "진짜 자율 에이전트(LLM이 도구·다음 단계를 스스로 결정)는 강력하지만 디버깅·예측 가능성·비용 면에서 비싸다."
    ),
    "sections": [
        {
            "title": "8.1 Augmented LLM — 모든 것의 시작",
            "content": (
                "기본 빌딩 블록: LLM + retrieval + tools + memory. 이 네 가지를 갖춘 단일 LLM 호출이 거의 모든 에이전트 시스템의 원자 단위다.\n\n"
                "- **Retrieval**: 외부 지식을 끌어다 context에 넣음.\n"
                "- **Tools**: LLM이 외부 함수를 호출 가능 (API, DB, 코드 실행).\n"
                "- **Memory**: 이전 turn·세션의 정보 유지.\n\n"
                "중요한 인사이트: 많은 'AI 시스템'이 이 단일 augmented LLM 호출 하나로 충분하다. 워크플로나 에이전트로 가기 전에, 단일 호출로 해결되는지 먼저 확인하라."
            )
        },
        {
            "title": "8.2 워크플로 패턴 — 미리 정의된 단계",
            "content": (
    ...
    "analogy": {
        ...
        "content": (
            ...
            "맥도날드를 오마카세 셰프로 운영할 이유 없다. 핵심 차별화가 셰프의 판단력에 있는 영역에서만 에이전트로. 그 외엔 워크플로로. "
            "Anthropic의 글은 이 결정을 명확히 하라고 한다. 모든 곳을 셰프로 만들면 빠르게 망한다."
        )
    },
    "codeExample": {
        "language": "python",
        ...
        "walkthrough": (
            "프레임워크 없이 50줄 안에 정상 동작하는 도구 사용 에이전트가 만들어진다. while 루프, max_steps, 명시적 종료 도구, "
            "도구 결과를 messages에 누적. Anthropic이 강조한 모든 원칙(simplicity, stopping criteria, transparency)이 이 작은 코드에 들어 있다. "
            "더 복잡한 에이전트도 이 핵심 패턴 위에 얹는다 — 프레임워크가 필요해지는 건 그다음이다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "AI 엔지니어 시니어 면접의 핵심 질문 — '이건 워크플로인가 에이전트인가?' 분류 능력이 설계 품질을 갈라놓는다."
        ),
        "whatEngineersLookFor": [
            "워크플로/에이전트 결정 기준이 명확",
        ...
        "masteryVsFamiliar": (
            "Familiar는 'ReAct·tool use 안다'고 한다. Mastery는 자기 시스템의 각 부분이 5가지 워크플로 패턴 중 무엇인지, "
            "어느 부분만 진짜 에이전트인지 정확히 설명할 수 있고, 그 결정의 ROI를 말할 수 있다."
        )
    },
    "keyTakeaways": [
        {"title": "Augmented LLM이 원자", "content": "LLM + retrieval + tools + memory. 모든 시작점."},
        {"title": "워크플로 vs 에이전트", "content": "단계가 정의됐나(워크플로) 아니면 LLM이 결정하나(에이전트)."},
        {"title": "5가지 워크플로 패턴", "content": "Chain·Routing·Parallel·Orchestrator·Evaluator."},
        {"title": "대부분은 워크플로로 충분", "content": "에이전트는 마지막 수단."},
        {"title": "단순함부터", "content": "프레임워크 없이 직접 짠 루프부터."},
        {"title": "Stopping criteria 필수", "content": "max_steps·timeout·비용 상한. 안전망."},
        {"title": "Transparency", "content": "왜 그렇게 행동했는지 trace로 답할 수 있어야."},
        {"title": "Context engineering", "content": "어떤 정보를 어느 시점에 줄지가 에이전트 품질을 결정."}
    ]
})

# ============================================================
# CHAPTER 9 — The Era of Experience (Sutton & Silver, 2024)
# ============================================================
chapters.append({
    "number": 9,
    "emoji": "🌅",
    "title": "Welcome to the Era of Experience",
    "titleKr": "경험의 시대 — David Silver & Rich Sutton (2024)",
    "tldr": "인간 데이터의 시대(LLM)는 끝나간다. 다음은 '경험의 시대' — 환경과 상호작용하며 스스로 데이터를 만드는 에이전트.",
    "topics": ["Era of Experience", "Streams of Experience", "Reward Grounding", "RL Renaissance", "Beyond Human Data"],
    "learningGoals": [
    ...
    "overview": (
        "David Silver(AlphaGo의 주역)와 Rich Sutton(Bitter Lesson 저자)이 2024년 11월에 함께 쓴 짧지만 강력한 에세이. 한 줄: "
        "'우리는 곧 인간 데이터의 한계에 도달한다. 다음은 에이전트가 환경과 상호작용하며 스스로 데이터를 만드는 경험의 시대다.'\n\n"
        "이 글은 4장 Bitter Lesson의 직접적 후속이다. Bitter Lesson이 '인간 지식이 결국 컴퓨팅에 진다'를 말했다면, 이 글은 '인간 데이터도 결국 한계가 있고, 경험이 그 너머로 간다'를 말한다. "
        "AI 에이전트 시대 개발자가 미래를 어디로 보고 시스템을 설계해야 하는지의 나침반이다."
    ),
    "sections": [
        {
            "title": "9.1 세 시대의 분류",
        ...
        {
            "title": "9.3 Streams of Experience",
            "content": (
                "그 다른 출처가 '경험 스트림(streams of experience)'이다.\n\n"
                "에이전트가 실제 환경과 상호작용하며 — 코드를 실행하고, 실패하고, 검색하고, 실험하고, 결과를 본다 — 자기 자신의 데이터를 만든다. "
                "AlphaZero가 self-play로 만든 데이터처럼, 단순히 인간이 쓴 글이 아니라 '시도와 결과'의 짝.\n\n"
                "이 데이터는 인간 데이터에 없는 두 가지를 가진다:\n"
                "(1) **Reward grounding**: 실제 환경 결과가 신호. 'compile passed', 'test failed', 'user accepted'. 객관적·검증 가능.\n"
                "(2) **인간을 넘을 가능성**: 인간이 시도하지 않은 경로도 탐색.\n\n"
                "예: Claude Code가 실제 코드베이스에서 PR을 제출하고, CI 결과를 받고, 리뷰를 받는 모든 경험이 에이전트의 학습 데이터가 된다. 인터넷 텍스트엔 없는 종류의 데이터다."
            )
        },
        {
            "title": "9.4 Reward Grounding의 어려움",
            "content": (
                "Era of Experience의 핵심 도전: '무엇이 좋은 결과인지'를 어떻게 정의하는가.\n\n"
                "체스·바둑은 쉽다 — 이기면 +1, 지면 -1. 명확하고 객관적인 보상.\n\n"
                "현실 세계는 어렵다. '좋은 코드'란? '좋은 응답'이란? '좋은 협업'이란? 인간이 명시적으로 정의해야 할 수도 있고, 인간 피드백을 학습해서 보상 모델을 만들 수도 있다. "
                "RLHF가 이 첫 시도였지만, 인간 데이터 시대의 흔적이다.\n\n"
    ...
    "analogy": {
        ...
        "content": (
            ...
            "Era of Experience의 에이전트는 실험실에서 자라는 과학자 같다. 가설을 세우고, 실험하고, 결과를 본다. 책도 읽지만, 더 중요한 건 자기 손으로 만든 데이터다. "
            "이 과학자는 책에 없는 발견을 할 수 있다 — 인간이 안 본 자리를 탐색하기 때문이다.\n\n"
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "이 루프의 핵심은 buffer에 쌓이는 (action, result, reward) 삼중주다. 인간이 쓴 텍스트가 아니라 '에이전트가 환경에서 직접 시도하고 받은 결과'가 데이터. "
            "이게 누적되면 RL fine-tuning이나 in-context 강화학습의 신호가 된다. Era of Experience의 정신을 코드 한 페이지로 응축한 모습이다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "Era of Experience는 아직 미래 전망이지만, '실험 환경·보상 정의·관측' 설계 능력은 이미 평가 항목이다."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'RL의 부활 같은 거지'라고 안다. Mastery는 자기 시스템의 어느 신호가 보상 후보이고, 어떤 sandbox가 필요하며, "
            "어떻게 telemetry를 환류시킬지를 구체적으로 설계할 수 있다."
        )
    },
    "keyTakeaways": [
        {"title": "세 시대", "content": "Simulation → Human Data → Experience."},
        {"title": "인간 데이터 한계", "content": "고갈·인간 천장·진실 부재·약한 reward signal."},
        {"title": "Streams of experience", "content": "환경 상호작용으로 자생한 데이터."},
        {"title": "Reward grounding", "content": "객관적·검증 가능한 환경 신호가 핵심."},
        {"title": "RL의 부활", "content": "self-play와 환경 학습이 다시 중심으로."},
        {"title": "Telemetry는 보상", "content": "우리 시스템 신호가 미래 에이전트 학습 자원."},
        {"title": "인간 역할 이동", "content": "라벨러에서 환경 설계자·보상 정의자로."},
        {"title": "Bitter Lesson의 후속", "content": "인간 지식 → 인간 데이터 → 두 한계 모두 컴퓨팅에 진다."}
    ]
})

# ============================================================
# CHAPTER 10 — Software Is Changing (Again) / Software 3.0 (Karpathy, 2025)
# ============================================================
chapters.append({
    "number": 10,
    "emoji": "🌐",
    "title": "Software Is Changing (Again) — Software 3.0",
    "titleKr": "소프트웨어가 또 변한다 — Andrej Karpathy (2025)",
    "tldr": "LLM은 새로운 컴퓨터다. 프롬프트가 새로운 프로그래밍 언어이고, 자연어가 인터페이스다. Vibe coding의 시대.",
    "topics": ["Software 3.0", "LLM as OS", "Prompt as Program", "Vibe Coding", "Autonomy Slider", "Human-AI Collaboration"],
    "learningGoals": [
    ...
    "overview": (
        ...
        "핵심 명제: 우리는 'Software 3.0' 시대에 들어섰다. 1.0은 코드, 2.0은 가중치, 3.0은 프롬프트. 자연어로 LLM을 프로그래밍한다. 이 강연은 1장의 'Software 2.0'에서 시작한 이 코스를 자연스럽게 닫는다 — "
        "같은 사람이 같은 사고 흐름으로 10년에 걸쳐 도달한 종착점이기 때문이다."
    ),
    "sections": [
        {
            "title": "10.1 세 가지 Software Layer",
            "content": (
        ...
        {
            "title": "10.4 Autonomy Slider — 인간/AI 협업 강도",
            "content": (
        ...
        {
            "title": "10.5 이 코스의 종착점 — 10년의 사고",
            "content": (
                "1장에서 Karpathy의 2017년 'Software 2.0'으로 시작한 이 코스가 같은 사람의 2025년 'Software 3.0'으로 닫힌다. 우연이 아니라 의도된 곡선이다.\n\n"
                "Karpathy의 사고 흐름을 따라가면: 'how → what (1.0 → 2.0)'에서 'what → why (2.0 → 3.0)'로 추상화가 한 단계 더 올라갔다. "
                "2.0에서 우리는 데이터(예시)를 줘서 함수를 발견시켰다. 3.0에서는 의도(자연어)를 줘서 동작을 발견시킨다.\n\n"
                "이 코스의 다른 글들도 이 곡선 위에 위치한다:\n"
                "- Sutton의 Bitter Lesson(4장)과 Era of Experience(9장) → '왜 3.0이 가능한가'의 이론.\n"
                "- Limoncelli(2장), Ousterhout(3장), McKinley(5장), Beck(6장) → '3.0 시대에도 살아남는 시대불문 원칙'.\n"
                "- LLM 빌드(7장), Anthropic 가이드(8장) → '3.0 시대 실전 패턴'.\n\n"
                "이 모든 글이 한 가지 메시지로 수렴한다: AI 시대 엔지니어의 일은 '추상화의 새 layer를 다루면서, 시대불문 원칙은 더 강하게 지키는 것'이다. "
                "도구는 변하지만, 단순함·자동화·복잡도 관리·점진성·평가의 가치는 변하지 않는다. 오히려 도구가 강해질수록 이 원칙들의 ROI가 폭증한다."
            )
        }
    ],
    "analogy": {
    ...
    "codeExample": {
        ...
        "walkthrough": (
            "1.0: 'how'가 코드에 있다. 새 카테고리 추가 → if 추가. 2.0: 'what'(데이터)으로 학습. 새 카테고리 추가 → 라벨링·재학습. "
            "3.0: 'why'(의도)를 자연어로. 새 카테고리 추가 → 프롬프트 문장 수정. 추상화 한 단계마다 변경의 노력이 한 자릿수씩 줄어든다. "
            "Karpathy의 10년 곡선이 이 한 페이지의 코드로 응축된다."
        )
    },
    "industryEvaluation": {
        "overview": (
            "AI 엔지니어 시니어 면접에서 'Vibe coding을 어디까지 허용하시나요'가 표준 질문이 되고 있다. Autonomy slider 결정 능력이 핵심."
        ),
        "whatEngineersLookFor": [
        ...
        "masteryVsFamiliar": (
            "Familiar는 'Karpathy가 vibe coding 얘기했다지'라고 안다. Mastery는 자기 시스템의 각 영역별로 autonomy slider 위치를 근거와 함께 설명할 수 있고, "
            "1.0/2.0/3.0 stack을 의도적으로 설계해서 각 layer의 강점을 활용한다."
        )
    },
    "keyTakeaways": [
        {"title": "세 layer stack", "content": "1.0 코드 + 2.0 가중치 + 3.0 프롬프트. 셋 다 살아 있다."},
        {"title": "LLM = 새 컴퓨터", "content": "Context = RAM, Tools = Bus, Agent harness = OS."},
        {"title": "Vibe coding", "content": "강력하지만 영역 제한 필요. 핵심 인프라엔 부적합."},
        {"title": "Autonomy slider", "content": "0~100% 연속. 영역별 다른 위치가 정상."},
        {"title": "Context engineering", "content": "Memory management의 새 분야."},
        {"title": "추상화 한 단계 상승", "content": "어셈블리 → 고수준 → 신경망 → 자연어."},
        {"title": "10년 사고의 닫힘", "content": "Karpathy 2017 → 2025. 'how → what → why'."},
        {"title": "변하지 않는 원칙", "content": "단순함·자동화·복잡도 관리. 도구가 강해질수록 더 중요."}
    ]
})

# Now write the final course data
print(f"Drafted {len(chapters)} chapters total")

data_preview = {
    "topic": "Engineering Philosophy in the AI Agent Era",
    "topicKr": "AI 에이전트 시대의 엔지니어링 철학",
    "topicSlug": "engineering-philosophy",
    "level": "intermediate",
    "codeLanguage": "Python",
    "description": (
        "2017년 이후, 특히 LLM과 AI 에이전트의 부상 이후에도 살아남고 오히려 더 중요해진 엔지니어링 철학 에세이 10편을 깊이 읽는다. "
        "Karpathy의 Software 2.0/3.0, Sutton의 Bitter Lesson과 Era of Experience, Anthropic의 Building Effective Agents 같은 AI 시대 정전부터 "
        "Limoncelli·Ousterhout·McKinley·Beck 같은 시대불문 거장의 철학까지. 각 장은 원문 정신을 한국어로 깊이 풀고, 코드 예시와 면접 평가 기준까지 담는다."
    ),
    "chapters": chapters
}

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
with open(OUTPUT_DIR / "course_data.json", "w", encoding="utf-8") as f:
    json.dump(data_preview, f, ensure_ascii=False, indent=2)

print(f"Wrote final to {OUTPUT_DIR}/course_data.json")
